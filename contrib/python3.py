...
def load_defaults():
    with hide('running'):
        env.python = local("{} python3".format(env.tools['which']), capture=True)

        # it's ok if we don't find these
        with settings(hide('warnings'), warn_only=True):
            # look in the python3 base directory
            env.python_virtualenv = local("{} virtualenv".format(env.tools['which']), capture=True)
            env.python_pip = local("{} pip3".format(env.tools['which']), capture=True)
